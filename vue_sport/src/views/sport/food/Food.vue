    ...
    <el-card>
      <el-row :gutter="25">
        ...
        <el-col :span="3" style="min-width: 100px">
          <el-button type="primary" @click="addShow">添加食物</el-button>
        </el-col>
        <el-col :span="3" v-has-role="['ROLE_ADMIN']" style="min-width: 100px">
          <el-upload
            :action="importUrl"
            :headers="headers"
            name="file"
          ...
          <el-row>
            <el-col :span="8">
              <el-form-item label="食物名称" prop="title">
                <el-input v-model="dataForm.title" type="textarea" autosize />
              </el-form-item>
            </el-col>
            <el-col :span="8">
    ...
    return {
      ...
      rulesDataForm: {
        ...
        nutrient: [
          {
            required: true,
            message: "请输入营养元素，如：每100g、每1杯、每1碗",
            trigger: "blur",
          },
        ],
        fat: [
  ...
  methods: {
    ...
    clickOk() {
      ...
      this.$refs.dataForm.validate((valid) => {
        if (!valid)
          return this.$message.error("表单校验不通过，请认真填写后提交！");
        //校验通过验证图片是否上传
        if (this.fileNames.length < 1)
          return this.$message.error("请上传食物图片，至少一张，至多4张！");
        ...
        if (this.dataForm.id === null || this.dataForm.id === undefined) {
          //添加
          this.$ajax
            .post("/food/insert", this.dataForm)
            .then((res) => {
              if (!res.data.flag) {
                setTimeout(() => {
                  this.loading = false;
          ...
          //修改
          this.$ajax
            .post("/food/update", this.dataForm)
            .then((res) => {
              if (!res.data.flag) {
                setTimeout(() => {
                  this.loading = false;
